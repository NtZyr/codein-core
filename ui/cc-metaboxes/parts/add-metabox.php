...
function cc_metabox( $box_num, $box = null ) {
    $box_id = ( $box[ 'id' ] ? $box[ 'id' ] : '' );
    $box_title = ( $box[ 'title' ] ? $box[ 'title' ] : '' );
    $box_types = ( $box['post_types'] ? $box['post_types'] : '' );
    $box_visibility = ( $box['visibility'] ? $box['visibility'] : '' );
    ...
    <div class="dropped-body">
        <div class="metabox_settings setting-body">
            <label>
                ...
                <input name="metaboxes[<?= $box_num; ?>][id]" value="<?= $box_id; ?>" type="text">
            </label>
            <label>
                <span><?= __( 'Metabox title', 'codein-core' ); ?></span>
                <input class="ajax-title" value="<?= $box_title; ?>" name="metaboxes[<?= $box_num; ?>][title]" type="text">
            </label>
            <label>
                <span><?= __( 'Post types', 'codein-core' ); ?></span>
                <select multiple name="metaboxes[<?= $box_num; ?>][post_types][]">
                    <?php foreach( $post_types as $name => $label ) : ?>

                    <option <?php if( is_array( $box_types ) ) { foreach( $box['post_types'] as $box_type ) { echo ( $box_type == $name ? 'selected' : '' ); } } ?> value="<?= $name; ?>"><?= $label; ?></option>
            ...
            <label>
                ...
                <select name="metaboxes[<?= $box_num; ?>][visibility]">
